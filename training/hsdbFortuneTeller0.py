    ...
    if os.path.isfile(path) == False:
        ...
        while i < len(lines)-3:
            ...
            if k % s == 0:
                if volOnly == False:
                    for k in range(depth):
                        datum.append(float(list(reversed(lines[i].split(",")[:depth]))[k])/scale)
                for k in range(depth):
                    datum.append(float(list(reversed(lines[i+1].split(",")[:depth]))[k])/scale)
                if volOnly == False:
                    for k in range(depth):
                        datum.append(float(list(reversed(lines[i+2].split(",")))[:depth][k])/scale)
                for k in range(depth):
    ...
    for i in range(lookback, len(dataset)-distance):
        ...
        try:
            ...
            if vO == False:
                ...
                c = (np.mean([dataset[i+distance][0], dataset[i+distance][depth]]) - np.mean([dataset[i][0], dataset[i][depth]]))
...
# TO-DO: fortune teller, predict future vol, predict future vwap
...
Din = 20; dist = 10; perc = 0.99; c = 1.5; b = 32; nb_epoch = 50; l = 20; opt = "adam"; s = 10; scale = 100000; vO = True
if vO == True:
    dims = Din*2*l
else:
    dims = Din*4*l
# TO-DO 50%: write formatted datasets to file, read from files on training to save reformatting time
# X, Y = create_forzaSequentialClassDirection_dataset(forza(path, Din, perc, s, scale, volOnly=vO), dist, Din, l, vO=vO)
X, Y = readDataset(path)
print("X0: ", X[0], " Y0 ", Y[0])
# plot(X[0])
# writeDirectionalDataset(X, Y, f'../../datasets/HSDBdirectionalClassLSTM0-MidPeak-Din{Din}-dist{dist}-lookback{l}-perc{perc}-sparcity{s}-scale{scale}-vO{vO}-dataset{path}')
#print("\nshape(X):", X.shape)
...
# lrs = ReduceLROnPlateau(monitor='loss', factor=0.95, patience=10, min_lr=0.0000000001)
