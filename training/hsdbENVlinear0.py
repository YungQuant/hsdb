...
Dsets = ["HSDBENVdirectionalClassLSTM0-MidPeak-Linear-Din3-dist6-lookback60-perc0.99-sparcity10-attention-6-interest0.5001-header669c-targetXBTUSD.txt",
         ...
         "HSDBENVdirectionalLSTM0-MidPeak-Linear-Din3-dist1-lookback60-perc0.99-sparcity60-attention-10-interest0.005-header669c-targetXBTUSD.txt"]
target = currencies[0]
Din = 3; dist = 1; perc = 0.99; c = 1.5; b = 32; nb_epoch = 50; l = 60; s = 60; scale = 10000000; vO = True
attention = 10; interest = 0.005; vFilter = "adaptive"; g = (10 if vFilter == "grouping" else 1); pF = True
...
for a in alphas:
    ...
    model = linear_model.ElasticNet(alpha=a, l1_ratio=0.5, normalize=False, precompute=False, tol=0.00000001)
    # model = svm.SVR()
    ...
    abas.append(passes / len(testY))
    mpes.append(np.mean(errs))

plot3(alphas, abas, mpes)
