    ...
    while(self->sync == true){
        if(self->quant.sync == true && self->quant.quant_sync == true){
            // try{
                ...
                (*self).quant.prep_book(writer, "ADAM19");
                (*self).quant.prep_book(writer, "BCHM19");
                (*self).quant.prep_book(writer, "EOSM19");
                (*self).quant.prep_book(writer, "ETHUSD");
                (*self).quant.prep_book(writer, "LTCM19");
                (*self).quant.prep_book(writer, "TRXM19");
                (*self).quant.prep_book(writer, "XRPM19");
                usleep(1000000);
                self->quant.updates++;
                std::cout << "Update Env: " << self->quant.updates << "\n";
