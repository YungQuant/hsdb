...
namespace SimpleWeb {
  ...
  protected:
    ...
    SocketClientBase(const std::string &host_port_path, unsigned short default_port) noexcept : handler_runner(new ScopeRunner()) {
      ...
      cout << "host_port_path " << host_port_path << " default_port " << default_port << endl;
    
      std::size_t host_end = host_port_path.find(':');
      std::size_t host_port_end = host_port_path.find('/');
      
      cout << "host_end " << host_end << " host_port_end " << host_port_end << endl;
      cout << "host_port_path.substr{host_end+1, host_port_end - {host_end + 1}} " << host_port_path.substr(host_end + 1, host_port_end - (host_end + 1)) << endl;
